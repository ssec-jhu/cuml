...
#include "sporfdecisiontree.cuh"              // class declaration
...
void validity_check(const DT::SPORFDecisionTreeParams params)
{
  ...
  ASSERT((params.density >= 0.0) && (params.density <= 1.0),
         "Invalid value for density: %d. Should be between 0.0 and 1.0",
         params.density);
}

/**
 * @brief Set all SPORFDecisionTreeParams members.
 ...
 */
void set_tree_params(DT::SPORFDecisionTreeParams& params,
                     int cfg_max_depth,
                     int cfg_max_leaves,
                     ...
                     HISTOGRAM_METHOD histogram_method)
{
  ...
  params.density               = density;           // SPORF only
  params.histogram_method      = histogram_method;  // SPORF only

  validity_check(params);
}

#ifdef TODO_IF_NEEDED_FOR_SPORF
...
#endif // TODO_IF_NEEDED_FOR_SPORF
