...
class SPORFRegressor(BaseRandomForestModel, RegressorMixin):
    """
    ...
    .. code-block:: python
        ...
        >>> import cupy as cp
        >>> from cuml.ensemble import SPORFRegressor as curfr
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # we cannot invoke super().__init__() (that is, BaseRandomForestModel.__init__()) using
        #  the given kwargs, which contain additional SPORF-specific parameters
        #  (see also SPORFClassifier.__init__())
        base_kwargs = kwargs
        del base_kwargs["density"]
        del base_kwargs["histogram_method"]
    ...
    @nvtx.annotate(
        message="fit RF-Regressor @sporfregressor.py",
        domain="cuml_python",
    )
    @generate_docstring()
    @reflect(reset=True)
    def fit(self, X, y, *, convert_dtype=True) -> "SPORFRegressor":
        """
        Perform SPORF Regression on the input data
