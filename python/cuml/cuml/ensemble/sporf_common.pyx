...
cdef extern from "cuml/ensemble/sporf.hpp" namespace "ML" nogil:
    ...
    # (the corresponding cdef for set_rf_params() is in randomforest_common.pyx)
...
class BaseSPORFModel(Base, InteropMixin):
    ...
    def __init__(
        ...
        *,
        ...
        verbose=False,
        output_type=None,
        density=1.0,        # SPORF only
        histogram_method=0    # SPORF only
    ):
    ...
    def _fit_forest(self, X, y):
        ...
        cdef SPORF_params params = set_sporf_params(
            self.max_depth,
            self.max_leaves,
            max_features,
            n_bins,
            ...
            self.n_streams,
            self.max_batch_size,
            self.density,          # SPORF only
            self.histogram_method, # SPORF only
        )
