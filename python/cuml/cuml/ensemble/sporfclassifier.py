...
from cuml.ensemble.sporf_common import BaseSPORFModel
...
class SPORFClassifier(BaseSPORFModel, ClassifierMixin):
    """
    Implements a Sparse Projection Oblique Random Forest regressor model which fits
    multiple decision tree classifiers in an ensemble.
    ...
    @nvtx.annotate(
        message="fit RF-Classifier @sporfclassifier.py",
        domain="cuml_python",
    )
    @generate_docstring(
    ...
    def fit(self, X, y, *, convert_dtype=True) -> "SPORFClassifier":
        """
        Perform SPORF Classification on the input data
        ...
        dtPrint("--> SPORFClassifier::fit() in sporfclassifier.py")
        ...
        y, classes = preprocess_labels(y, n_samples=X_m.shape[0], dtype=cp.int32)
    ...
    @nvtx.annotate(
        message="predict RF-Classifier @sporfclassifier.py",
        domain="cuml_python",
    )
    ...
    @nvtx.annotate(
        message="score RF-Classifier @sporfclassifier.py",
        domain="cuml_python",
    )
    @insert_into_docstring(
        parameters=[
